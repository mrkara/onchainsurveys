									...
									<table role="table" class="table table-hover table_link" id="all_surveys" >
										...
										<?php foreach ($list as $key => $value) {?>
											 <tr role="row" onclick="go('survey/detail/<?=id_encode($value->survey_id);?>');">
													...
													<td role="cell">
														<a href="survey/detail/<?=id_encode($value->survey_id); ?>">
															 <?=$value->title; ?>
														</a>
													</td>
													<td role="cell">
														<span><?=@get_user($value->user_id,'user_name')->user_name; ?></span>
													</td>
													<td role="cell">
														<span>
															<?php 
														...
														?> 
	 													<?=$start_date_arr[0];?> 
	 													<span class="surver_time"><?=$start_date_arr[1];?></span></span> <br>
	 													<span>
	 														<?php 
	 														# end date explode
													...
													<td role="cell" style="min-width:130px;">
														<div>
															<?php 
															# şuanki zaman
															$now = date('Y-m-d H:i:s');
													...
													<td role="cell">
														<div>
															<?php if(is_superUser()){?>
															<a href="survey/survey_edit/<?=id_encode($value->survey_id);?>"><button class="btn btn-outline-secondary btn-sm editBtn"> <i class="fas fa-edit"></i></button></a>
														<?php } ?>
														</div>
